...
import useKeylistener, { useWithModifiersKeylistener } from "../hooks/useKeylistener";
import {
	FaCog,
	FaHandPaper,
	FaMinus,
...
function VideoPlayer(props: VideoPlayerProps): JSX.Element {
	...
	const next = () => {
		if (
			play &&
			(currentMarkerIndex >= (videos[currentIndex]?.config?.markers?.length ?? -1) ||
	...
	const controlByMarkers = (seconds: number) => {
		...
		const markerIndex = videos[currentIndex]?.config?.markers.findIndex(
	...
	const playLocalFiles = (fileList: FileList | null) => {
		...
		const videos: VideoFile[] = fileArray.map((file) => ({
	...
	const resetConfig = (config: Config) => {
		...
		setVideos((videos): VideoFile[] =>
			videos.map(
				(v): VideoFile => ({
					config: {
						markers: config?.find((c) => c.fileName === v.fileName)?.markers ?? [],
					},
					...v,
				}),
			),
